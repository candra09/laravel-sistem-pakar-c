...
class DiagnosaController extends Controller
{
    public function __construct()
    {
        $this->middleware(function ($request, $next) {
            if (Auth::check() && (Auth::user()->hasRole('Admin') || Auth::user()->hasRole('Pengguna'))) {
                return $next($request);
            }
            ...
            foreach($final as $key => $value) {
                ...
                if(count($final) - 1 == $key) {
                    if($cf_max == null) {
                        $cf_max = [
                            $hasil_cf,
                            "{$final[0]->nama} ({$final[0]->kode})",
                            $final[0]->solusi ?? 'Data tidak tersedia',
                            $final[0]->penyebab ?? 'Data tidak tersedia',
                            $final[0]->pencegahan ?? 'Data tidak tersedia'
                        ];
                    } else {
    ...
    public function diagnosa(Request $request)
    {
       ...
       $pdf =  PDF::loadView('pdf.riwayat', ['id' => $riwayat->id])
            ->save($path."/".$file_pdf);
